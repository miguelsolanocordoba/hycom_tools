...
c 3D variables
     ...
     & blk_ubar,blk_vbar
      ...
      dimension blk_ubar(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension blk_vbar(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
...
c 2D variables
     ...
     & kes,apes,keaver,apeaver,
     & kea1,kea2,kea3,kea4,apea1,apea2,apea3,apea4,
     & keadvx,apeadvx,keadvbarx,apeadvbarx,
     & keadvy,apeadvy,keadvbary,apeadvbary
      ...
      dimension kea1(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension kea2(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension kea3(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension kea4(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension keadvx(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension keadvbarx(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension keadvy(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension keadvbary(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension kes(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension keaver(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension apea1(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension apea2(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension apea3(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension apea4(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension apeadvx(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension apeadvbarx(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension apeadvy(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
      dimension apeadvbary(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy)
...
!depth integrated advection of KE due to wave field (u')
        outfile12='out/' 
     &  //'keadvx_'//runid//
     &'_'//trim(ftnm)//'_'
     &  //numstr(jblk)//'_'//numstr(iblk)//'.BinF' 
...
!depth integrated advection of APE due to wave field (u')
        outfile13='out/' 
     &  //'apeadvx_'//runid//
     &'_'//trim(ftnm)//'_'
     &  //numstr(jblk)//'_'//numstr(iblk)//'.BinF' 
...
!depth integrated advection of KE due to mean field (ubar)
        outfile14='out/' 
     &  //'keadvbarx_'//runid//
     &'_'//trim(ftnm)//'_'
     &  //numstr(jblk)//'_'//numstr(iblk)//'.BinF'  
...
!depth integrated advection of APE due to mean field (ubar)
        outfile15='out/' 
     &  //'apeadvbarx_'//runid//
     &'_'//trim(ftnm)//'_'
     &  //numstr(jblk)//'_'//numstr(iblk)//'.BinF' 
...
!depth integrated advection of APE due to wave field (u')
     ...
     &  //numstr(jblk)//'_'//numstr(iblk)//'.BinF'
        ...
        open(unit=117,file=outfile17,status='replace',
     &      form='unformatted',access='sequential')
...
! initialize variables ("a" is mean, "s" is std)
        kea1(:,:) = 0.0  
        kea2(:,:) = 0.0  
        kea3(:,:) = 0.0  
        kea4(:,:) = 0.0  
...
! time-consuming? 
              ...
              blk_ubar(i,j)  = blk_ubarof(i,j,l)
              blk_vbar(i,j)  = blk_vbarof(i,j,l)
...
! fixed it by making sure drhodz is not zero
     ...
     &                   /(drhodz(i,j,k)+drhodz(i,j,k+1))
                  ...
                  apeadvx(i,j) = apeadvx(i,j) - Mbar
     &                         *  0.5*(blk_u(i,j,k) + blk_u(i+1,j,k))
                  apeadvy(i,j) = apeadvy(i,j) - Mbar
     &                         *  0.5*(blk_v(i,j,k) + blk_v(i,j+1,k))
                  apeadvbarx(i,j) = apeadvbarx(i,j) - Mbar
     &                         *  0.5*(blk_ubar(i,j) + blk_ubar(i+1,j))
                  apeadvbary(i,j) = apeadvbary(i,j) - Mbar
     &                         *  0.5*(blk_vbar(i,j) + blk_vbar(i,j+1))

                endif ! apemeth 

! Advection of KE due to baroclinic velocity (u'*KE)
                  keadvx(i,j) =  keadvx(i,j) + blk_thknss(i,j,k)
     &                  *   (1000.0+blk_sigt(i,j,k))*0.25
     &                  *   (blk_u(i,j,k)**2 + blk_u(i+1,j,k)**2  
     &                  +    blk_v(i,j,k)**2 + blk_v(i,j+1,k)**2)
     &                  *   0.5*(blk_u(i,j,k)+ blk_u(i+1,j,k)) 

                  keadvy(i,j) =  keadvy(i,j) + blk_thknss(i,j,k)
     &                  *   (1000.0+blk_sigt(i,j,k))*0.25
     &                  *   (blk_u(i,j,k)**2 + blk_u(i+1,j,k)**2  
     &                  +    blk_v(i,j,k)**2 + blk_v(i,j+1,k)**2)
     &                  *   0.5*(blk_v(i,j,k)+ blk_v(i,j+1,k)) 
...
! Advection of KE due to barotropic velocity (ubar*KE)
                  keadvbarx(i,j) =  keadvbarx(i,j) + blk_thknss(i,j,k)
     &                  *   (1000.0+blk_sigt(i,j,k))*0.25
     &                  *   (blk_u(i,j,k)**2 + blk_u(i+1,j,k)**2  !atfirst p
     &                  +    blk_v(i,j,k)**2 + blk_v(i,j+1,k)**2)
     &                  *   0.5*(blk_ubar(i,j)+ blk_ubar(i+1,j)) 

                  keadvbary(i,j) =  keadvbary(i,j) + blk_thknss(i,j,k)
     &                  *   (1000.0+blk_sigt(i,j,k))*0.25
     &                  *   (blk_u(i,j,k)**2 + blk_u(i+1,j,k)**2  !atfirst p
     &                  +    blk_v(i,j,k)**2 + blk_v(i,j+1,k)**2)
     &                  *   0.5*(blk_vbar(i,j)+ blk_vbar(i,j+1)) 
...
! Advection of APE due to baroclinic velocity (u'*APE)
                 apeadvx(i,j) = apeadvx(i,j) + grav_r
     &                  *  0.5*(blk_u(i,j,k) + blk_u(i+1,j,k))
     &                 * (eta_anom(i,j,k)**2) 

                 apeadvy(i,j) = apeadvy(i,j) + grav_r
     &                  *  0.5*(blk_v(i,j,k) + blk_v(i,j+1,k))
     &                 * (eta_anom(i,j,k)**2) 


! Advection of APE due to barotropic velocity (ubar'*APE)
                  apeadvbarx(i,j)=apeadvbarx(i,j)+grav_r 
     &            * 0.5*(blk_ubar(i,j)+blk_ubar(i+1,j))
     &                 * (eta_anom(i,j,k)**2) 

                  apeadvbary(i,j)=apeadvbary(i,j)+grav_r 
     &            * 0.5*(blk_vbar(i,j)+blk_vbar(i,j+1))
     &                 * (eta_anom(i,j,k)**2) 
...
! keep track of the time-mean fields, avoid polluted parts due to ringing
                 if ((l.GT.RILE).AND.(l.LE.maxobs-RILE)) then
                   apea1(i,j) = apea1(i,j)+apeadvx(i,j)
                   apea2(i,j) = apea2(i,j)+apeadvbarx(i,j)
                   apea3(i,j) = apea3(i,j)+apeadvy(i,j)
                   apea4(i,j) = apea4(i,j)+apeadvbary(i,j)
                   kea1(i,j)  = kea1(i,j)+keadvx(i,j)
                   kea2(i,j)  = kea2(i,j)+keadvbarx(i,j)
                   kea3(i,j)  = kea3(i,j)+keadvy(i,j)
                   kea4(i,j)  = kea4(i,j)+keadvbary(i,j)
...
! finish computing mean
        apea1  = apea1/float(maxobs-2*RILE)
        apea2  = apea2/float(maxobs-2*RILE)
        apea3  = apea3/float(maxobs-2*RILE)
        apea4  = apea4/float(maxobs-2*RILE)
        kea1   = kea1/float(maxobs-2*RILE)
        kea2   = kea2/float(maxobs-2*RILE)
        kea3   = kea3/float(maxobs-2*RILE)
        kea4   = kea4/float(maxobs-2*RILE)
...
! finish computing std (not for dissipation)
        ...
        write(6,*)'maxval apea',maxval(apea1),
     &                maxobs-2*RILE
        write(6,*)'maxval kea',maxval(kea1),
     &                maxobs-2*RILE
        write(6,*)'maxval apea',maxval(apea2),
     &                maxobs-2*RILE
        write(6,*)'maxval kea',maxval(kea2),
     &                maxobs-2*RILE
        write(6,*)'maxval apea',maxval(apea3),
     &                maxobs-2*RILE
        write(6,*)'maxval kea',maxval(kea3),
     &                maxobs-2*RILE
        write(6,*)'maxval apea',maxval(apea4),
     &                maxobs-2*RILE
        write(6,*)'maxval kea',maxval(kea4),
     &                maxobs-2*RILE
