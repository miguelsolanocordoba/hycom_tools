...
!   plot rotary spectra from surface and v velocities
!
      implicit none
...
! ---2d+t variables-----
      ...
      real, allocatable :: blk_u_surfu(:,:,:),blk_v_surfu(:,:,:)
      real, allocatable :: blk_u_tseries(:,:,:),blk_v_tseries(:,:,:),
     &             blk_ubaro_tseries(:,:,:),blk_vbaro_tseries(:,:,:)

      real blk_mean_u,blk_mean_v
...
! allocate variables
     ...
     & blk_u_surfu(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy,1:maxobs),
     & blk_v_surfu(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy,1:maxobs),
     & blk_u_tseries(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy,1:maxobs),
     & blk_v_tseries(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy,1:maxobs),
     & blk_ubaro_tseries(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy,1:maxobs),
     & blk_vbaro_tseries(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy,1:maxobs),
     & blk_cw(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy,1:maxobs/2),
     & blk_ccw(1-mbdy:blki+mbdy,1-mbdy:blkj+mbdy,1:maxobs/2))
...
!----------------- Reading barotropic and baroclinic velocities ------------------------------
          blk_u_surf(:,:,:)=0.0
          blk_v_surf(:,:,:)=0.0
          blk_u_surfu(:,:,:)=0.0
          blk_v_surfu(:,:,:)=0.0
         
          blk_ubaro_tseries(:,:,:)=0.0
          blk_vbaro_tseries(:,:,:)=0.0
          ...
          do l=1,maxobs
            ...
            blk_ubaro_tseries(:,:,l)=blk_ubaro(:,:)
            blk_vbaro_tseries(:,:,l)=blk_vbaro(:,:)
...
! taking only the surface value, k = 1
            blk_u_tseries(:,:,l)=blk_u(:,:,1)
            blk_v_tseries(:,:,l)=blk_v(:,:,1)
         ...
         blk_u_surfu = blk_u_tseries + blk_ubaro_tseries
         blk_v_surfu = blk_v_tseries + blk_vbaro_tseries

!------------------ Moving velocities to p points ------------------

              do j=2-mbdy,blkj+mbdy-1
                do i=2-mbdy,blki+mbdy-1
                  blk_u_surf(i,j,:)=
     &              0.5*(blk_u_surfu(i,j,:) + blk_u_surfu(i+1,j,:))
                  blk_v_surf(i,j,:)=
     &              0.5*(blk_v_surfu(i,j,:) + blk_v_surfu(i,j+1,:))
                enddo ! j
               enddo ! i
...
!                 write(6,*) i,j
           ...
           t1(l)=dble(l) ! time array for rot spec subroutine
...
!---------Calling rot_spec_fftw------------------------------------------
     ...
     &                 (max(1,size(blk_v_surf(i,j,:))))
              blk_v_surf(i,j,:)= blk_v_surf(i,j,:)-blk_mean_v  !subtract mean       
                ...
                u1 = dble(blk_u_surf(i,j,:))  ! velocity time series
                v1 = dble(blk_v_surf(i,j,:))  !  for rot spec subroutine
...
! Store only half of the double-sided spectra                 
                blk_cw(i,j,:) = cw(maxobs:maxobs/2 + 1:-1)
                blk_ccw(i,j,:) = ccw(maxobs:maxobs/2 + 1:-1)
             enddo   !i
           enddo     !j 
...
!-----------write output--------------------------------------------------
      ...
      do l=1,maxobs/2 ! half of double-sided spectra
       write(110) blk_cw(:,:,l)
       write(111) blk_ccw(:,:,l)
      enddo
